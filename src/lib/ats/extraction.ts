...
async function extractTextFile(file: File): Promise<ExtractedResume> {
  ...
  return { text, pageCount: 1, source: "text" };
}
...
async function extractDocxFile(file: File): Promise<ExtractedResume> {
  ...
  if (!text) {
    throw new Error("We could not extract readable text from this DOCX.");
  }
  return { text, pageCount: 1, source: "docx" };
}
...
export async function extractResumeText(file: File): Promise<ExtractedResume> {
  ...
  if (isPdfFile(file)) return extractPdfFile(file);
  if (isWordFile(file)) return extractDocxFile(file);
  if (isTextFile(file)) return extractTextFile(file);
