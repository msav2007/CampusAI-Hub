...
export function analyzeResumeAgainstJob(resumeText: string, jobDescription: string): AtsAnalysis {
  ...
  const jdKeywords = jobDescription ? extractTargetKeywords(jobDescription) : SKILLS_DB.slice(0, 15); // Fallback to general skills
