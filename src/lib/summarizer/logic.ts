export type SummaryMode =
  "short" | "detailed" | "bullets" | "revision" | "flashcards" | "questions";
  ...
  switch (mode) {
    ...
    case "flashcards": {
      ...
      if (flashcards.length < 5) {
        ...
        const candidates = rankedSentences
          .slice(0, needed + flashcards.length)
          .map((s) => s.sentence);
        candidates.forEach((c) => {
          // ensure not already added
          if (!flashcards.some((f) => f.a === c)) {
            const importantWord =
              keyTopics.find((t) => c.toLowerCase().includes(t)) || keyTopics[0];
            flashcards.push({
    ...
    case "questions": {
      ...
      topSentences.forEach((s, i) => {
        ...
        if (matchTopic) {
          if (i % 3 === 0)
            questions.push(`Explain the concept of ${matchTopic} and its implications.`);
          else if (i % 3 === 1)
            questions.push(`What are the key characteristics of ${matchTopic}?`);
