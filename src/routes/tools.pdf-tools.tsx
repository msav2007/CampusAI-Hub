...
export const Route = createFileRoute("/tools/pdf-tools")({
  head: () => ({
    ...buildPageHead({
      ...
      description:
        "Visual PDF Editor. Render, annotate, merge, split, extract text, and watermark PDFs entirely in your browser.",
...
function PdfToolsPage() {
  ...
  return (
    <ToolShell
      ...
      title={
        <>
          PDF <span className="text-gradient">Studio Pro</span>
        </>
      }
      description="Visual PDF editing, annotating, merging, and extraction. 100% private."
    >
          ...
          <div className="flex flex-col gap-3 bg-surface-2/30 p-3 rounded-xl border border-border/50">
            ...
            <div className="flex gap-2 items-center flex-wrap">
              ...
              <Input
                ...
                placeholder="Size"
              />
              <Button size="sm" onClick={() => setEdits([])} variant="ghost">
                Undo All
              </Button>
              <Button size="sm" onClick={processEdits} disabled={edits.length === 0}>
                Save PDF
              </Button>
            </div>
          </div>

          <div className="overflow-auto border rounded-xl border-border/50 bg-foreground/5 mx-auto w-full max-h-[600px] flex justify-center">
            <canvas
              ref={canvasRef}
  ...
  return (
    <div className="rounded-2xl glass p-6 shadow-card flex flex-col items-center justify-center min-h-[400px] text-center gap-4">
      ...
      <p className="text-muted-foreground max-w-md">
        Highlight, draw, and underline directly in your browser. Use the{" "}
        <strong>Viewer & Editor</strong> tool for basic text additions.
      </p>
      <div className="text-sm bg-amber-500/10 text-amber-500 p-3 rounded-lg border border-amber-500/20">
        Advanced freehand annotations (drawing pens, precise highlights) require a dedicated WebGL
        context which is currently in development for local processing.
      </div>
    </div>
  );
}
  ...
  const processWatermark = async () => {
    ...
    if (mode === "remove") {
      toast.error("Watermark Removal Failed", {
        description:
          "Watermarks are permanently burned into the PDF layout stream. Native removal requires a server-side layout engine. Cannot safely extract.",
      });
      return;
    }
    toast.loading("Applying...", { id: "wm" });
    try {
      const blob = await watermarkPdf(files[0], text, {
        opacity,
          ...
          {mode === "add" ? (
            <div className="space-y-4">
              ...
              <select
                ...
                className="w-full h-10 rounded-md border border-input bg-surface-2/40 px-3 text-sm shadow-sm outline-none"
              >
              ...
              />
              <Button onClick={processWatermark} className="w-full">
                Apply Watermark
              </Button>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="text-sm bg-rose-500/10 text-rose-500 p-3 rounded-lg border border-rose-500/20">
                <strong>Heuristic Detection Active:</strong> Local engines cannot distinguish
                baked-in image watermarks from actual page content.
              </div>
              <Button onClick={processWatermark} variant="destructive" className="w-full">
                Attempt Removal
              </Button>
            </div>
          )}
  ...
  return (
    <div className="rounded-2xl glass p-6 shadow-card flex flex-col items-center justify-center min-h-[400px] text-center gap-4">
      ...
      <div className="text-sm bg-amber-500/10 text-amber-500 p-4 rounded-lg border border-amber-500/20 max-w-md">
        <strong>Limitation Explained:</strong> Local zero-backend engines (like <code>pdf-lib</code>
        ) do not natively support encrypting or setting passwords on PDFs during creation due to
        WebCrypto API limitations in worker scopes. Adding passwords requires a server-side
        component.
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl glass p-6 shadow-card flex flex-col items-center justify-center min-h-[400px] text-center gap-4">
      ...
      <p className="text-muted-foreground">
        Add images or extract images from PDFs. (Currently optimizing WebAssembly memory limits for
        high-res extraction).
      </p>
      <div className="text-sm bg-amber-500/10 text-amber-500 p-4 rounded-lg border border-amber-500/20 max-w-md">
        <strong>Status:</strong> Advanced image extraction is limited locally due to memory
        constraints in browser Web Workers. Basic support coming soon.
      </div>
    </div>
  );
}
    ...
    <div className="rounded-2xl glass p-6 shadow-card min-h-[500px] flex flex-col gap-6">
      ...
      {files.length >= 2 && (
        <Button onClick={processMerge} disabled={isLoading} className="w-full mt-auto">
          {isLoading ? "Processing..." : `Merge ${files.length} PDFs`}
        </Button>
      )}
    </div>
  );
                  ...
                  {Array.from({ length: pdfInfo.pages }).map((_, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-10 rounded-lg border flex items-center justify-center font-medium transition-colors text-sm ${isSelected ? "bg-brand text-brand-foreground border-brand" : "bg-surface-2/50 border-border/50 hover:border-brand/50"}`}
                      >
                        {pageNum}
                      </button>
                    );
...
function DeletePagesTool() {
  ...
  const processDelete = async () => {
    ...
    try {
      ...
      toast.error("Failed to delete pages.", { id: "pdf-del" });
    } finally {
      setIsLoading(false);
    }
  };
              ...
              {Array.from({ length: pdfInfo.pages }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`h-10 rounded-lg border flex items-center justify-center font-medium transition-colors text-sm ${isSelected ? "bg-destructive text-destructive-foreground border-destructive" : "bg-surface-2/50 border-border/50 hover:border-brand/50"}`}
                  >
                    {pageNum}
                  </button>
                );
...
function ReorderPagesTool() {
  ...
  const processReorder = async () => {
    ...
    try {
      ...
      toast.error("Failed to reorder pages.", { id: "pdf-reorder" });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="rounded-2xl glass p-6 shadow-card min-h-[500px] flex flex-col gap-6">
      ...
      {file && pdfInfo && (
        <div className="flex flex-col gap-6 flex-1 mt-4">
          ...
          <Button onClick={processRotate} disabled={isLoading} className="w-full mt-auto">
            Rotate PDF
          </Button>
        </div>
      )}
...
function ExtractTool() {
  ...
  const processExtract = async () => {
    ...
    try {
      ...
      toast.error("Failed to extract.", { id: "pdf-ext" });
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="rounded-2xl glass p-6 shadow-card flex flex-col h-[500px]">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs text-muted-foreground">
              {extractedText.trim().split(/\s+/).length} words extracted
            </span>
            <div className="flex gap-2">
              <Button
                variant="outline"
