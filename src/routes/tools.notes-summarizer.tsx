...
export const Route = createFileRoute("/tools/notes-summarizer")({
  head: () => ({
    ...buildPageHead({
      ...
      description:
        "Convert weak lecture notes into detailed summaries, flashcards, exam questions, and study guides. Runs locally in your browser.",
      path: "/tools/notes-summarizer",
      keywords:
        "notes assistant, summarize notes, local ai summarizer, student study tools, flashcards generator",
    }),
  }),
...
function NotesAssistantPage() {
  ...
  return (
    <ToolShell
      ...
      title={
        <>
          Notes <span className="text-gradient">Assistant</span>
        </>
      }
              ...
              <div className="flex flex-col gap-6 flex-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="bg-surface-2/50 rounded-xl p-3 border border-border/50 flex flex-col justify-center">
                    ...
                    <span className="text-lg font-bold text-foreground">
                      {result.analytics.readingTimeMin} min
                    </span>
                  </div>
                  <div className="bg-surface-2/50 rounded-xl p-3 border border-border/50 flex flex-col justify-center">
                    <span className="text-[10px] uppercase text-muted-foreground font-semibold">
                  ...
                  <div className="bg-surface-2/50 rounded-xl p-3 border border-border/50 flex flex-col justify-center">
                    ...
                    <span
                      className={`text-lg font-bold ${
                        result.analytics.difficulty === "Advanced"
                          ? "text-destructive"
                          : result.analytics.difficulty === "Intermediate"
                            ? "text-amber-500"
                            : "text-green-500"
                      }`}
                    >
                ...
                <div className="bg-surface-2/30 rounded-xl border border-border/50 p-4">
                  <span className="text-xs uppercase text-muted-foreground font-semibold block mb-2">
                    Key Topics Detected
                  </span>
                ...
                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                  <div className="bg-surface-2/30 rounded-xl border border-border/50 p-5 prose prose-sm dark:prose-invert max-w-none">
                    {result.output.split("\n").map((line, i) => {
                      if (line.startsWith("###")) {
                        return (
                          <h3 key={i} className="text-lg font-bold text-foreground mt-0 mb-4">
                            {line.replace("### ", "")}
                          </h3>
                        );
                      }
                      ...
                      if (line.startsWith("**Q") && line.includes("**:")) {
                        ...
                        return (
                          <div key={i} className="font-bold text-foreground mb-1">
                            {parts[0]}**:{" "}
                            <span className="font-normal text-muted-foreground">{parts[1]}</span>
                          </div>
                        );
                      }
                      if (line.startsWith("**A") && line.includes("**:")) {
                        const parts = line.split("**: ");
                        return (
                          <div key={i} className="font-bold text-brand mb-1">
                            {parts[0]}**:{" "}
                            <span className="font-normal text-foreground">{parts[1]}</span>
                          </div>
                        );
                      }
